...
DMZ internet-facing log ingestion gateway for O.A.S.I.S.
...
```
Internet → External Gateway (DMZ) → Ingestion Service (Backend)
                ↓
        PostgreSQL (auth)
```
...
```bash
docker build -t oasis-external-gateway:latest .
```
